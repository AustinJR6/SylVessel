...
gcloud run deploy sylana-vessel \
  ...
  --cpu 1 \
  --timeout 300 \
  --set-env-vars CLAUDE_MODEL=claude-sonnet-4-5-20250929,CORS_ORIGINS=* \
  --set-secrets ANTHROPIC_API_KEY=anthropic-api-key:latest,SUPABASE_DB_URL=supabase-db-url:latest
```
